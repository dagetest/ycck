 ...
 */
public class GuanWangShouYeChuYiChuErShuXue {
    private static Log logger = LogFactory.getLog(GuanWangShouYeChuYiChuErShuXue.class); //打印日志
    public static void main(String[] args) throws InterruptedException {
        ...
        try {
            ...
            logger.error("官网首页初一初二数学getFreeLesson出异常了");
        }
        DBTools4Student.allowDuplicateRegiste(mobile);//清除手机号
        try {
            ...
            System.out.println("出异常了 = [" + args + "]");
            logger.error("官网首页初一初二数学getFreeLesson1 出异常了");
        }
        DBTools4Student.allowDuplicateRegiste(mobile);//清除手机号
        try {
            ...
            System.out.println("出异常了 = [" + args + "]");
            logger.error("官网首页初一初二数学getFreeLesson2 出异常了");
        }
        DBTools4Student.allowDuplicateRegiste(mobile);//清除手机号
        try {
            ...
            System.out.println("出异常了 = [" + args + "]");
            logger.error("官网首页初一初二数学getFreeLesson3 出异常了");
        }
        DBTools4Student.allowDuplicateRegiste(mobile);//清除手机号
//        logger.debug("自动化程序运行成功，浏览器退出");
//        logger.info("自动化程序运行成功，浏览器退出");
//        logger.warn("自动化程序运行成功，浏览器退出");
//        logger.error("自动化程序运行成功，浏览器退出");
        driver.quit();
    }
    private static void getFreeLesson(WebDriver driver,String mobile) throws InterruptedException {
        ...
        Thread.sleep(2000);
        By by = By.xpath("//*[@id=\"lessonSystem\"]/div/ul[1]/li[2]/a");  //定位到初一初二
        WebElement element = driver.findElement(by);
        Actions action = new Actions(driver);   //创建一个动作
        action.moveToElement(element).build().perform();  //移动到元素上
        //点击初一初二数学
        //*[@id="lessonSystem"]/div/ul[2]/li[1]/a
        driver.findElement(By.xpath("//*[@id=\"lessonSystem\"]/div/ul[2]/li[1]/a")).click();
        Thread.sleep(2000);
        ...
        if(strUrl2.equals(driver.getCurrentUrl())){
            logger.debug("官网首页初一初二数学课程，第1个0元试听失败！");
        }else{
            logger.debug("官网首页初一初二数学课程，第1个0元试听成功！");
        }
    }
    private static void getFreeLesson1(WebDriver driver,String mobile) throws InterruptedException {
        ...
        String strUrl2 = "https://v2-test.zm1v1.com/appointment-success";
        if(strUrl2.equals(driver.getCurrentUrl())){
            logger.debug("官网首页初一初二数学课程，第2个0元试听失败！");
        }else{
            logger.debug("官网首页初一初二数学课程，第2个0元试听成功！");
        }
    }
    private static void getFreeLesson2(WebDriver driver,String mobile) throws InterruptedException {
        ...
        String strUrl2 = "https://v2-test.zm1v1.com/appointment-success";
        if(strUrl2.equals(driver.getCurrentUrl())){
            logger.debug("官网首页初一初二数学课程，第3个0元试听失败！");
        }else{
            logger.debug("官网首页初一初二数学课程，第3个0元试听成功！");
        }
    }
    private static void getFreeLesson3(WebDriver driver,String mobile) throws InterruptedException {
        ...
        String strUrl2 = "https://v2-test.zm1v1.com/appointment-success";
        if(strUrl2.equals(driver.getCurrentUrl())){
            logger.debug("官网首页初一初二数学课程，第4个0元试听失败！");
        }else{
            logger.debug("官网首页初一初二数学课程，第4个0元试听成功！");
        }
    }
}
